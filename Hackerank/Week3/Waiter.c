...
int* waiter(int number_count, int* number, int q, int* result_count) {
    ...
    int* stack_A = malloc(number_count * sizeof(int));
    int top_A = -1;
    int* stack_B = malloc(number_count * sizeof(int));
    int top_B = -1;
    int* next_stack_A = malloc(number_count * sizeof(int));
    int top_next_A = -1;

    for (int i = 0; i < number_count; i++) {
        stack_A[++top_A] = number[i];
    }
    ...
    for (int i = 0; i < q; i++) {
        ...
        while (top_A > -1) {
            int plate = stack_A[top_A--]; 
            if (plate % current_prime == 0) {
                stack_B[++top_B] = plate; 
            } else {
                next_stack_A[++top_next_A] = plate; 
            }
        }
        
        
        while (top_B > -1) {
            answers[answers_idx++] = stack_B[top_B--];
        }

        
        int* temp_stack = stack_A;
        stack_A = next_stack_A;
        top_A = top_next_A;
        
        
        next_stack_A = temp_stack;
        top_next_A = -1;
    }
    
    
    while (top_A > -1) {
        answers[answers_idx++] = stack_A[top_A--];
    }
